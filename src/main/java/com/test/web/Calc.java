	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(!x_.equals("")) { //사용자에게 입력 받은 값을 조건검사, 만약 x 값이 ""(공백)이 아니라면
			if(y_.equals("")) { //만약 y 값이 ""(공백)이라면 
				...
				if(op.equals("덧셈")) //만약 넘겨받은 값이 덧셈이라면
					result = x+y;
				if(op.equals("뺄셈")) //만약 넘겨받은 값이 뺄셈이라면
					result = x-y;
				
				out.println("계산 결과는 "+result+"입니다");

			}
		}else if(!y_.equals("")){ //29행 조건을 만족하지 않고 y 값이 ""(공백) 이 아니라면
			out.println("x 값을 입력해주세요");			
		}else { // 29행, 45행 조건을 다 만족하지 않는다면 --> x, y 둘 다 공백이라면
